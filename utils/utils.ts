...
export function startDatePerWeekday(beginDate: Dayjs, weekday: ICalWeekday): Dayjs {
  const beginWeekday = beginDate.day();
  const wdNum = weekdayNumValue[weekday];
  if (beginWeekday <= wdNum) { return beginDate.day(wdNum); }
  return beginDate.day(7 + wdNum)
}

export function closestStartDate(weekdays: ICalWeekday[]) {
  const dates: Dayjs[] = weekdays.map((wd) => startDatePerWeekday(startDate[nowSem], wd));
