import { eventCellTextToIntermediateEventData, IntermediateEventData, intermediateEventDatatoIcalEvent } from "@/utils/parse";
...
function getTimeFromRow(row: number) {
  ...
  return { start: dummyStart.add((30 * (row - 1)), 'minute'), end: dummyEnd.add((30 * (row - 1)), 'minute') };
}
...
function createCalButton(calendar: ICalCalendar) {
  ...
  calLink.setAttribute('href', 'data:text/calendar;charset=utf8,' + encodeURIComponent(calendar.toString()));
  calLink.setAttribute('download', 'class_schedule.ics');

  return calLink;
}

const weekdays = [ICalWeekday.MO, ICalWeekday.TU, ICalWeekday.WE, ICalWeekday.TH, ICalWeekday.FR, ICalWeekday.SA];
...
export default defineContentScript({
  ...
  main() {
    if (window.location.href === "https://aisis.ateneo.edu/j_aisis/J_VMCS.do") {
      const schedtable: HTMLTableElement | null = document.querySelector('[width="90%"]');
      if (schedtable) {
        const tableBody = schedtable.tBodies[0];
        const rows: HTMLCollectionOf<HTMLTableRowElement> = tableBody.getElementsByTagName('tr');
        ...
        for (let row = 1; row < rows.length; ++row) {
          for (let col = 1; col < 7; ++col) {
            ...
            const eventData = eventCellTextToIntermediateEventData(content, weekdays[col - 1], time.start, time.end);
        ...
        const simplifiedEventDatas: IntermediateEventData[] = simplifySchedule(sched);

        const calendar = ical();

        const events: ICalEvent[] = [];
        for (const eventData of simplifiedEventDatas) {
          const subject = (eventData.subject === undefined) ? '' : eventData.subject;
          events.push(intermediateEventDatatoIcalEvent('', subject, calendar, eventData));
        }
